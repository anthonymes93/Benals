  ...
  return (
    <div className="relative overflow-hidden bg-navy-950">
      <Container className="grid grid-cols-1 items-center gap-10 py-14 sm:py-20 lg:grid-cols-2 lg:gap-16 lg:py-28">
        <div>
          {eyebrow ? (
            <p className="mb-4 text-sm font-semibold tracking-wide text-brand-red-400 uppercase">
              {eyebrow}
            </p>
          ) : null}
            ...
            <ul className="mt-9 flex flex-wrap gap-x-6 gap-y-2 text-sm text-navy-200">
              {trustPoints.map((point) => (
                <li key={point} className="flex items-center gap-2">
                  <span className="h-1.5 w-1.5 rounded-full bg-brand-red-500" aria-hidden="true" />
                  {point}
                </li>
              ))}
            </ul>
